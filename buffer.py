...
def main(args):
    ...
    trajectories = []  # 存储训练过程中模型的参数变化

    dst_train = TensorDataset(copy.deepcopy(images_all.detach()), copy.deepcopy(labels_all.detach()))  #　创建一个新的数据集，复制images_all和labels_all，detach()返回的tensor和原始tensor共同一个内存，但是该tensor的requires_grad永远为false
    ...
    args.dc_aug_param['strategy'] = 'crop_scale_rotate'  # for whole-dataset training 设置数据增强策略：裁剪，缩放，旋转
    print('DC augmentation parameters: \n', args.dc_aug_param)

    ## 模型训练循环，每次迭代都会训练一个合成数据模型
    for it in range(0, args.num_experts):
        # 训练合成数据
        ''' Train synthetic data '''
        teacher_net = get_network(args.model, channel, num_classes, im_size).to(args.device)  # get a random model
        teacher_net.train()  # 设置为训练模式，从而启用模型的训练特性，如Dropout等
        lr = args.lr_teacher
